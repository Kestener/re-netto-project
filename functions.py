...
def unique_customers_and_combine_files(path='./', file_type="csv"):
    """
    Reads in the CSV files for customer behavior from monday to friday and concatenates them into a single DataFrame with rolling customer numbering.
    Returns the concatenated DataFrame.
    """
    # Read in the five CSV files
    df_monday = pd.read_csv(f"{path}monday.csv", sep=';', parse_dates=True, index_col=0, skipinitialspace=True)
    df_tuesday = pd.read_csv(f"{path}tuesday.csv", sep=';', parse_dates=True, index_col=0, skipinitialspace=True)
    df_wednesday = pd.read_csv(f"{path}wednesday.csv", sep=';', parse_dates=True, index_col=0, skipinitialspace=True)
    df_thursday = pd.read_csv(f"{path}thursday.csv", sep=';', parse_dates=True, index_col=0, skipinitialspace=True)
    df_friday = pd.read_csv(f"{path}friday.csv", sep=';', parse_dates=True, index_col=0, skipinitialspace=True)
